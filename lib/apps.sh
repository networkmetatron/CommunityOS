...
if declare -F service_sync_compose_profiles >/dev/null 2>&1; then service_sync_compose_profiles; fi
  # COMPOSE_PROFILES from .env selects website|chat|ai stacks; infra has no profile
  docker compose "${args[@]}" --env-file "${COMMUNITYOS_ROOT}/.env" "$@" "${extra[@]}"
}


# Ensure runtime/dnsmasq.conf is a regular file (never a directory).
# Docker creates a host *directory* if a bind-mounted file path is missing
# when a container starts — remove that mistake before writing config.
ensure_dnsmasq_conf_file() {
  local conf="${1:-${COMMUNITYOS_ROOT}/runtime/dnsmasq.conf}"
  mkdir -p "$(dirname "${conf}")"
  if [[ -d "${conf}" ]]; then
    if declare -F log_warn >/dev/null 2>&1; then
      log_warn "Removing mistaken directory at ${conf} (expected a file)"
    fi
    rm -rf "${conf}"
  fi
  if [[ ! -e "${conf}" ]]; then
...
app_update_dns() {
  ...
  # All locals initialized — safe under set -u.
  local conf="${COMMUNITYOS_ROOT}/runtime/dnsmasq.conf"
  local ip=""
  local provide_dns="0"
  ensure_dnsmasq_conf_file "${conf}"
  ...
  provide_dns="${provide_dns%\"}"; provide_dns="${provide_dns#\"}"
  case "${provide_dns}" in
    1|true|TRUE|yes|YES|Y|y) provide_dns="1" ;;
    *) provide_dns="0" ;;
  esac
  ...
  # Ensure derived names exist even if domain_load left gaps
  DOMAIN_BASE="${DOMAIN_BASE:-community.home.arpa}"
  DOMAIN_CHAT="${DOMAIN_CHAT:-chat.${DOMAIN_BASE}}"
  DOMAIN_AI="${DOMAIN_AI:-ai.${DOMAIN_BASE}}"
  ...
  # Only publish DNS names for enabled core services and installed optional apps.
  local hosts=()
  # Website uses DOMAIN_BASE; include when website enabled OR no service flags (legacy full)
  _dns_core_on() {
    local id="$1"
  ...
  if _dns_core_on website; then
    hosts+=("${DOMAIN_BASE}")
  fi
  if _dns_core_on chat; then
    hosts+=("${DOMAIN_CHAT}")
  fi
  if _dns_core_on ai; then
    hosts+=("${DOMAIN_AI}")
  fi
  # Optional apps only when enabled
  if [[ -f "${COMMUNITYOS_ROOT}/runtime/apps/kiwix.enabled" ]]; then
    hosts+=("${DOMAIN_LIBRARY}")
  fi
  if [[ -f "${COMMUNITYOS_ROOT}/runtime/apps/maps.enabled" ]]; then
    hosts+=("${DOMAIN_MAPS}")
  fi
...

  {
    ...
    echo "# Hostnames limited to enabled services / installed apps."
  ...
  # Reload DNS only if the container is already up. Callers such as
  # `communityos start` rewrite conf *before* compose up — do not warn then.
...
# ---------------------------------------------------------------------------
# Core service Compose profiles (website | chat | ai)
# Infrastructure services (caddy, dns) have no profile and always start.
# ---------------------------------------------------------------------------

service_profiles_from_flags() {
  local profiles=()
  if [[ -f "${COMMUNITYOS_ROOT}/runtime/services/website.enabled" ]] || \
     { [[ ! -d "${COMMUNITYOS_ROOT}/runtime/services" ]] && [[ ! -f "${COMMUNITYOS_ROOT}/.env" ]]; }; then
    :
  fi
  # Prefer runtime flags when the directory has been seeded
  if [[ -d "${COMMUNITYOS_ROOT}/runtime/services" ]] && compgen -G "${COMMUNITYOS_ROOT}/runtime/services/*.enabled" >/dev/null 2>&1; then
    [[ -f "${COMMUNITYOS_ROOT}/runtime/services/website.enabled" ]] && profiles+=("website")
    [[ -f "${COMMUNITYOS_ROOT}/runtime/services/chat.enabled" ]] && profiles+=("chat")
    [[ -f "${COMMUNITYOS_ROOT}/runtime/services/ai.enabled" ]] && profiles+=("ai")
  else
    # Fall back to .env SERVICE_* (default all true for legacy installs)
    local sw sc sa
    sw=true; sc=true; sa=true
    if [[ -f "${COMMUNITYOS_ROOT}/.env" ]]; then
      # shellcheck disable=SC1090
      set -a; source "${COMMUNITYOS_ROOT}/.env" 2>/dev/null || true; set +a
      sw="${SERVICE_WEBSITE:-true}"
      sc="${SERVICE_CHAT:-true}"
      sa="${SERVICE_AI:-true}"
    fi
    case "${sw}" in 1|true|TRUE|yes|Y|y) profiles+=("website") ;; esac
    case "${sc}" in 1|true|TRUE|yes|Y|y) profiles+=("chat") ;; esac
    case "${sa}" in 1|true|TRUE|yes|Y|y) profiles+=("ai") ;; esac
  fi
  # Optional apps that need profiled shared infra without enabling a core stack.
  # PeerTube depends on postgres (compose profiles: chat, peertube).
  if [[ -f "${COMMUNITYOS_ROOT}/runtime/apps/peertube.enabled" ]]; then
    profiles+=("peertube")
  fi
  # Hermes depends on ollama (compose profiles: ai, hermes) without Open WebUI.
  if [[ -f "${COMMUNITYOS_ROOT}/runtime/apps/hermes.enabled" ]]; then
    profiles+=("hermes")
  fi
  # If nothing selected, still allow infrastructure-only
  local IFS=,
  echo "${profiles[*]-}"
}

# Write COMPOSE_PROFILES into .env (docker compose reads this automatically)
service_sync_compose_profiles() {
  local profiles
  profiles="$(service_profiles_from_flags)"
  local envf="${COMMUNITYOS_ROOT}/.env"
  [[ -f "${envf}" ]] || return 0
  if grep -q '^COMPOSE_PROFILES=' "${envf}" 2>/dev/null; then
    sed -i "s|^COMPOSE_PROFILES=.*|COMPOSE_PROFILES=${profiles}|" "${envf}"
  else
    echo "COMPOSE_PROFILES=${profiles}" >> "${envf}"
  fi
