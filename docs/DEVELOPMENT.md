...
## Domain configuration (`DOMAIN_BASE`)

CommunityOS uses a single source of truth for hostnames:

```bash
DOMAIN_BASE=community.home.arpa   # default
# or
DOMAIN_BASE=community.example.com
```

Derived automatically (do not set independently unless debugging):
...
Helpers live in `lib/domain.sh` (`domain_load`, `domain_derive`, `domain_for`, `domain_validate`).

Installer prompts for local `.home.arpa` vs custom domain. Existing `.env` values are preserved on upgrade and are never silently reset to the default.

Caddy, dnsmasq, CLI `info`/`doctor`, and optional-app metadata all read these variables.

## Domain changes after install

```bash
sudo communityos domain status
sudo communityos domain set community.example.com
sudo communityos domain set community.home.arpa
```

Updates `DOMAIN_BASE`, derived hostnames, DNS, Caddy, and Element config.
A backup is written under `backups/domain-*`. Matrix homeserver identity inside
existing Synapse data is sensitive when renaming Chat.

## Modular core services

Install-time selection writes `SERVICE_WEBSITE`, `SERVICE_CHAT`, `SERVICE_AI` and
`runtime/services/*.enabled`. Manage later with:

```bash
communityos service status
sudo communityos service enable chat
sudo communityos service disable website
```

## Doctor DNS checks
When CommunityOS DNS is running, `communityos doctor` queries **only** that resolver (`dig @SERVER_IP` / `127.0.0.1` / in-container dig). It does not fall back to public recursive DNS, which can report unrelated public A records for the same name.
