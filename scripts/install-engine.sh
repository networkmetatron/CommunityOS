...
if [[ ! -f "${TARGET}/runtime/dnsmasq.conf" ]]; then
  cat > "${TARGET}/runtime/dnsmasq.conf" <<'DNSPLACEHOLDER'
# CommunityOS DNS placeholder — replaced during install
domain-needed
bogus-priv
no-resolv
server=1.1.1.1
...
COMMUNITYOS_OFFLINE="${COMMUNITYOS_OFFLINE:-0}"
COMMUNITYOS_OFFLINE_BUNDLE="${COMMUNITYOS_OFFLINE_BUNDLE:-}"

if [[ "${COMMUNITYOS_OFFLINE}" == "1" ]]; then
  # shellcheck source=/dev/null
  if [[ -f "${PKG_DIR}/lib/offline.sh" ]]; then
    source "${PKG_DIR}/lib/offline.sh"
  elif [[ -f "${TARGET}/lib/offline.sh" ]]; then
    source "${TARGET}/lib/offline.sh"
  else
    echo "[FAIL] Offline mode requested but lib/offline.sh is missing."
    exit 1
  fi

  if [[ -z "${COMMUNITYOS_OFFLINE_BUNDLE}" || ! -d "${COMMUNITYOS_OFFLINE_BUNDLE}" ]]; then
    echo "[FAIL] COMMUNITYOS_OFFLINE_BUNDLE is not set to a valid directory."
    echo "       Usage: sudo ./install.sh --offline /path/to/communityos-offline-vX.Y.Z"
    exit 1
  fi

  offline_verify_bundle "${COMMUNITYOS_OFFLINE_BUNDLE}"
fi
...
ensure_bootstrap_tools() {
  ...
  if [[ "${COMMUNITYOS_OFFLINE}" == "1" ]]; then
    # libnss3-tools should already be in the offline package set.
    true
  else
...
# shellcheck source=/dev/null
# PKG_DIR / BASE_DIR are set above; never expand an unbound ROOT_DIR under set -u
if [[ -f "${PKG_DIR}/lib/domain.sh" ]]; then
  source "${PKG_DIR}/lib/domain.sh"
elif [[ -f "${BASE_DIR}/lib/domain.sh" ]]; then
  source "${BASE_DIR}/lib/domain.sh"
elif [[ -f "$(cd "$(dirname "$0")/.." && pwd)/lib/domain.sh" ]]; then
  source "$(cd "$(dirname "$0")/.." && pwd)/lib/domain.sh"
else
  echo "[FAIL] lib/domain.sh not found"
  exit 1
fi
...
# --- TLS mode (local CA default; ACME DNS-01 optional for public custom domains) ---
TLS_MODE="${TLS_MODE:-local}"
if [[ "${_domain_prompt:-1}" -eq 1 ]] && ! domain_is_home_arpa "${DOMAIN_BASE}"; then
  echo
  echo "Certificate mode for ${DOMAIN_BASE}:"
  echo "  1) CommunityOS local CA (default — works offline / private LAN)"
  echo "  2) Public certificate — Let's Encrypt via Cloudflare DNS-01"
  echo
  read -rp "Choice [1]: " _tls_choice
  case "${_tls_choice:-1}" in
    2)
      TLS_MODE="acme_dns01"
      echo
      echo "Selected: public Let's Encrypt (ACME DNS-01)"
      echo
      echo "  DNS provider: Cloudflare must be authoritative for ${DOMAIN_BASE}"
      echo "  Validation:   DNS-01 (LAN-only OK; no public port 80/443 required)"
      echo "  Client trust: ordinary browsers — no CommunityOS ca.crt install"
      echo
      echo "  Cloudflare API token needs: Zone → DNS → Edit (this zone only)."
      echo "  Domain can stay registered elsewhere; only NS must be Cloudflare."
      echo "  DNS records may be DNS-only (grey cloud); proxy is optional."
      echo
      echo "  After install, create:"
      echo "    ${BASE_DIR}/secrets/acme.env   (chmod 600, root:root)"
      echo "      ACME_EMAIL=..."
      echo "      ACME_DNS_PROVIDER=cloudflare"
      echo "      CLOUDFLARE_API_TOKEN=..."
      echo "      ACME_WILDCARD=1"
      echo
      echo "  Then:"
    ...
    *)
      TLS_MODE="local"
      echo "Selected: CommunityOS local CA"
      ;;
  esac
  echo
fi
# .home.arpa and offline always force local
if domain_is_home_arpa "${DOMAIN_BASE}"; then
  TLS_MODE="local"
fi
...
SERVICE_AI=true
_service_prompt=1
if [[ -f "${BASE_DIR}/.env" ]] && grep -q '^SERVICE_WEBSITE=' "${BASE_DIR}/.env" 2>/dev/null; then
  _service_prompt=0
fi
if [[ "${_service_prompt}" -eq 1 ]]; then
  echo
  echo "CommunityOS services (core):"
  echo
  echo "  1) Website"
  echo "  2) Chat"
  echo "  3) AI Assistant"
  echo
  echo "Enter selections (comma-separated). Example: 1,3"
  echo "Press Enter for all services (recommended)."
  echo
  read -rp "Selections [1,2,3]: " SERVICE_CHOICE
  SERVICE_CHOICE="${SERVICE_CHOICE:-1,2,3}"
  SERVICE_WEBSITE=false
  SERVICE_CHAT=false
  SERVICE_AI=false
  # Accept "all" or empty already handled
  IFS=',' read -ra _sel <<< "${SERVICE_CHOICE}"
  for _s in "${_sel[@]}"; do
    _s="$(echo "${_s}" | tr -d '[:space:]')"
    case "${_s}" in
      1|website|Website) SERVICE_WEBSITE=true ;;
      2|chat|Chat)       SERVICE_CHAT=true ;;
      3|ai|AI|assistant) SERVICE_AI=true ;;
      all|All) SERVICE_WEBSITE=true; SERVICE_CHAT=true; SERVICE_AI=true ;;
    esac
  done
  # Require at least one
  if [[ "${SERVICE_WEBSITE}" != true && "${SERVICE_CHAT}" != true && "${SERVICE_AI}" != true ]]; then
    echo "[WARN] No services selected — enabling all."
    SERVICE_WEBSITE=true
    SERVICE_CHAT=true
    SERVICE_AI=true
  fi
  echo
  echo "Selected:"
  [[ "${SERVICE_WEBSITE}" == true ]] && echo "  • Website"
  [[ "${SERVICE_CHAT}" == true ]] && echo "  • Chat"
  [[ "${SERVICE_AI}" == true ]] && echo "  • AI Assistant"
  echo
  echo "You can enable or disable services later:"
  echo "  sudo communityos service enable|disable <website|chat|ai>"
  echo
fi
...
if ! command -v docker >/dev/null 2>&1; then
  ...
  if [[ "${COMMUNITYOS_OFFLINE}" == "1" ]]; then
    ...
    if ! command -v docker >/dev/null 2>&1; then
      echo "[FAIL] Docker was not installed from the offline bundle packages."
      echo "       Rebuild the bundle on a matching Debian ${VERSION_ID:-13} host."
      exit 1
    fi
  else
    apt-get update -qq
...
# Offline: load Docker images from the bundle (never docker pull)

if [[ "${COMMUNITYOS_OFFLINE}" == "1" ]]; then
  offline_load_images "${COMMUNITYOS_OFFLINE_BUNDLE}"

  # Assert core images from compose.yaml are present
  mapfile -t _core_imgs < <(offline_core_images "${BASE_DIR}")
  offline_assert_images_present "${_core_imgs[@]}"
fi
...
# Service enable flags (modular core) — before DNS so hostnames match selection
mkdir -p "${BASE_DIR}/runtime/services"
rm -f "${BASE_DIR}/runtime/services/"*.enabled
_profiles=()
[[ "${SERVICE_WEBSITE}" == true ]] && { touch "${BASE_DIR}/runtime/services/website.enabled"; _profiles+=("website"); }
[[ "${SERVICE_CHAT}" == true ]] && { touch "${BASE_DIR}/runtime/services/chat.enabled"; _profiles+=("chat"); }
[[ "${SERVICE_AI}" == true ]] && { touch "${BASE_DIR}/runtime/services/ai.enabled"; _profiles+=("ai"); }
# Compose reads COMPOSE_PROFILES from .env — only selected stacks are created/started
IFS=','; _cp="${_profiles[*]-}"; IFS=$' \t\n'
if grep -q '^COMPOSE_PROFILES=' "${BASE_DIR}/.env" 2>/dev/null; then
  sed -i "s|^COMPOSE_PROFILES=.*|COMPOSE_PROFILES=${_cp}|" "${BASE_DIR}/.env"
else
  echo "COMPOSE_PROFILES=${_cp}" >> "${BASE_DIR}/.env"
fi
export COMPOSE_PROFILES="${_cp}"
echo "[INFO] Compose profiles: ${COMPOSE_PROFILES:-'(infrastructure only)'}"

# DNS — only hostnames for enabled core services
# Docker bind-mounts ./runtime/dnsmasq.conf; if that path is missing when a
# container starts, Docker creates a *directory* on the host. Always force a file.
mkdir -p "${BASE_DIR}/runtime"
_dns_conf="${BASE_DIR}/runtime/dnsmasq.conf"
if [[ -d "${_dns_conf}" ]]; then
  echo "[WARN] Removing mistaken directory ${_dns_conf} (expected a config file)"
  rm -rf "${_dns_conf}"
fi
{
  echo "# CommunityOS LAN DNS — DOMAIN_BASE=${DOMAIN_BASE}"
  echo "# Hostnames limited to selected services at install time."
  echo "domain-needed"
  echo "bogus-priv"
  echo "no-resolv"
  echo "server=1.1.1.1"
  echo "server=8.8.8.8"
  [[ "${SERVICE_WEBSITE}" == true ]] && echo "address=/${DOMAIN_BASE}/${SERVER_IP}"
  [[ "${SERVICE_CHAT}" == true ]] && echo "address=/${DOMAIN_CHAT}/${SERVER_IP}"
  [[ "${SERVICE_AI}" == true ]] && echo "address=/${DOMAIN_AI}/${SERVER_IP}"
} > "${_dns_conf}"
if [[ ! -f "${_dns_conf}" ]]; then
...
if [[ "${COMMUNITYOS_OFFLINE}" == "1" ]]; then
  COMPOSE_PULL_FLAG=(--pull never)
else
  COMPOSE_PULL_FLAG=()
fi

if [[ "${PROVIDE_DNS}" -eq 1 ]]; then
  docker compose -f "${BASE_DIR}/compose.yaml" --env-file "${BASE_DIR}/.env" up -d "${COMPOSE_PULL_FLAG[@]}"
else
...
# Wait for selected services only
for _ in $(seq 1 18); do
  _ready=1
  if [[ "${SERVICE_WEBSITE}" == true ]]; then
    docker exec communityos-caddy wget -q -O /dev/null http://wordpress:80/ >/dev/null 2>&1 || _ready=0
  fi
  if [[ "${SERVICE_CHAT}" == true ]]; then
    docker exec communityos-caddy wget -q -O /dev/null http://synapse:8008/health >/dev/null 2>&1 || _ready=0
  fi
  if [[ "${SERVICE_AI}" == true ]]; then
    docker inspect --format='{{.State.Status}}' communityos-open-webui 2>/dev/null | grep -qi running || _ready=0
  fi
  [[ "${_ready}" -eq 1 ]] && break
  sleep 5
done
...
if [[ "${HEALTH_FAIL}" -eq 0 ]]; then
  ...
  echo "Some selected services are still starting. Wait a minute, then open the welcome page."
fi
echo
...
if [[ "${TLS_MODE:-local}" == "local" ]]; then
  ...
  echo "${_step_cert}. Public certificates (Let's Encrypt via ACME DNS-01)."
  echo "   Ordinary browsers trust these automatically — no CA install required."
  echo "   Ensure ${BASE_DIR}/secrets/acme.env has ACME_EMAIL, ACME_DNS_PROVIDER, and API token."
  echo
  echo "${_step_https}. Use HTTPS:"
fi
echo
[[ "${SERVICE_WEBSITE}" == true ]] && echo "      https://${DOMAIN_BASE}"
[[ "${SERVICE_CHAT}" == true ]] && echo "      https://${DOMAIN_CHAT}"
[[ "${SERVICE_AI}" == true ]] && echo "      https://${DOMAIN_AI}"
echo
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo
...
if [[ "${PROVIDE_DNS}" -eq 0 ]]; then
  ...
  if docker exec communityos-dns true >/dev/null 2>&1; then
    ...
    if command -v dig >/dev/null 2>&1; then
      ...
      if is_ipv4 "${ans}"; then
        # Only verify hostnames for services that were selected
        _dns_names=()
        [[ "${SERVICE_WEBSITE}" == true ]] && _dns_names+=("${DOMAIN_BASE}")
        [[ "${SERVICE_CHAT}" == true ]] && _dns_names+=("${DOMAIN_CHAT}")
        [[ "${SERVICE_AI}" == true ]] && _dns_names+=("${DOMAIN_AI}")
        [[ "${#_dns_names[@]}" -eq 0 ]] && _dns_names+=("${DOMAIN_BASE}")
...
# Persist offline mode so later app installs still avoid network pulls

if [[ "${COMMUNITYOS_OFFLINE}" == "1" ]]; then
  mkdir -p "${BASE_DIR}/runtime"
  cat > "${BASE_DIR}/runtime/offline.env" <<OFF
# Generated by CommunityOS offline installer — do not edit unless you know why
COMMUNITYOS_OFFLINE=1
COMMUNITYOS_OFFLINE_BUNDLE=${COMMUNITYOS_OFFLINE_BUNDLE}
OFF
  echo "[ OK ] Offline mode recorded in runtime/offline.env"
fi
